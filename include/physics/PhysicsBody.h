        ...
        for (int z = z0; z <= z1; ++z) {
            for (int y = y0; y <= y1; ++y) {
                for (int x = x0; x <= x1; ++x) {
                    ...
                    if (axis == 0) {
                        if (delta > 0.0f) newPos = std::min(newPos, blk.min.x - halfSize.x - voxel_physics::EPS);
                        else              newPos = std::max(newPos, blk.max.x + halfSize.x + voxel_physics::EPS);
                    } else if (axis == 1) {
                        if (delta > 0.0f) newPos = std::min(newPos, blk.min.y - halfSize.y - voxel_physics::EPS);
                        else              newPos = std::max(newPos, blk.max.y + halfSize.y + voxel_physics::EPS);
                    } else {
                        if (delta > 0.0f) newPos = std::min(newPos, blk.min.z - halfSize.z - voxel_physics::EPS);
                        else              newPos = std::max(newPos, blk.max.z + halfSize.z + voxel_physics::EPS);
                    }
                }
            }
        }
